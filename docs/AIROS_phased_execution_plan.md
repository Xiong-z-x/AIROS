...
Native sensor note: `sim.launch.py` defaults to `sensor_source:=native`. `/scan` is bridged from Gazebo `gpu_lidar` as `LaserScan`, and `/livox/lidar` is bridged from Gazebo `gpu_lidar` `PointCloudPacked` output via `/livox/lidar/points` into ROS `/livox/lidar`. The ROS-side emulators remain available as `sensor_source:=emulated` fallback.
