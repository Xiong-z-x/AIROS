...
Transitional note: /scan is published by airos_experiments scan_emulator, not by Gazebo GPU lidar. The emulator is required because ogre2 crashes on this machine and gpu_lidar under ogre publishes zero ranges.
FAST-LIO2 visual upgrade: implemented on 2026-05-07. `visual_fast_lio_navigation.launch.py` launches Gazebo, FAST-LIO2, Nav2, and RViz. `/livox/lidar`, `/livox/imu`, `/cloud_registered`, `/Laser_map`, and `/Odometry` are live. RViz RobotModel now uses Transient Local robot_description QoS, all 14 robot links are reachable from map, and RViz keeps the Nav2 `/map` display entry plus the point-cloud map display.
