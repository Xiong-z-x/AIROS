...
- FAST-LIO2 `/Laser_map_world` 到 terrain planner 的链路已验证为同层可规划，并已具备跨层目标的前沿探索机制：高层目标暂不可达时，规划器会保留最终目标，先发布当前 FAST-LIO2 已建图连通区域内的 `/pct_path`，并在检测到高处已建图结构时把探索方向偏向该结构。完整跨楼层到达仍未验收，不能声称已经完成专题级全链路复现。
