...
AIROS 是当前 WSL2 / Ubuntu 22.04 / ROS 2 Humble / Gazebo Fortress 环境下的 Go2W 风格自主导航原型。项目已从早期平面单层 Nav2 验证推进到 FAST-LIO2 SLAM 点云建图、SLAM-cloud traversability map、PCT-style 跨层路径生成和安全门控运动链。

新会话接手时先读 `docs/handoff/README.md`。较早的路线、计划和进度文档仍保留历史价值；如果它们与当前代码、测试或 `docs/handoff/` 不一致，以当前代码/测试和 `docs/handoff/` 为准。
...
- 当前 FAST-LIO/PCT demo 主控制链：`/cmd_vel_nav -> velocity_smoother -> collision_monitor -> diff_drive_controller -> /odom -> TF`。传统 Nav2 full stack 仍作为稳定平面基线保留。
...
- FAST-LIO2 地形规划入口：`terrain_pct_planner` 默认在 `terrain_map_source:=slam_cloud` 下订阅对齐后的 `/Laser_map_world`，从 FAST-LIO2 点云地图抽取 `/terrain_traversability_cloud`，并从 SLAM map 生成 `/pct_path`。
- FAST-LIO2 局部安全入口：累计 `/Laser_map_world` 只作为 SLAM 规划地图，实时 `/cloud_registered` 会对齐为 `/cloud_registered_world` 并投影成 `/slam_scan`，供 collision monitor 和前沿局部障碍阻塞使用。
...
- FAST-LIO2 `/Laser_map_world` 到 terrain planner 的链路已验证为可生成高层 `/pct_path`：最新在线证据中 `/Laser_map_world` 从 69075 点增长到 594777 点，`/pct_path` 最高约 `z=2.155`，`/cmd_vel_nav` 和 `/odom` 链路持续有效。机器人物理到达高层平台仍未验收，不能声称已经完成专题级全链路复现。

## 当前推荐展示路径

当前最快、最稳、最适合展示的结果是单层 FAST-LIO2 + PCT-style + direct
tracking 全链路：
...
该脚本会清理残留运行态、headless 启动 FAST-LIO2 可视化导航主链、发布
`/terrain_goal_pose`、采集 `/Laser_map_world`、`/pct_path`、`/cmd_vel_nav`、
smoothed command、base command 和 Gazebo pose，并用 Gazebo/wheel 到点距离作为
物理验收信号。

最新单层展示证据：

- `log/fast_lio_single_floor_demo/near_goal_after_scan_index_20260516_223232/`：
  final Gazebo/wheel goal distance `0.222402m`。
- `log/fast_lio_single_floor_demo/long_corridor_after_scan_index_20260516_223440/`：
  final Gazebo/wheel goal distance `0.294605m`，`/pct_path_poses=10`，
  `/pct_path_max_z=-0.044546`。
- 两个 run 均有 `/Laser_map_world` 增长、base command 输出和 0 个
  `/slam_scan` stale warning。

最终单层冻结报告见
`docs/handoff/single_floor_final_report_2026-05-16.md`。后续接手时先保持该
展示链稳定，不要再把跨层物理爬升作为当前展示阻塞项。

跨层当前只达到部分验收：live SLAM 增长、高层 `/pct_path` 和 direct command
chain 都已复现，但 Gazebo 物理 pose 尚未爬到高层平台。继续跨层前应先改变输入
条件，例如更小的 multilevel smoke world、ramp-capable model/control 分支，或
先由短坡道 smoke 验证的 ramp-entry 执行修复；不要在同一轮式 surrogate 和同一大
地图上重复长时间试跑。
...
- `docs/handoff/README.md`：新会话和接手时的事实源入口。
- `docs/handoff/fast_display_and_next_actions_2026-05-16.md`：当前最快展示路线、跨层停止条件和后续最小工作包。
- `docs/handoff/objective_completion_audit_2026-05-16.md`：当前目标完成度审计，明确单层已可展示、跨层未完成。
