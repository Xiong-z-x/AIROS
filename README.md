...
- ROS 侧 `/scan` 仿真：从 SDF 静态障碍和可选动态障碍生成 `LaserScan`，发布 QoS 兼容 RViz、Nav2 和 collision monitor。
- ROS 侧 Livox/点云仿真：发布 `/livox/lidar`、`/livox/imu`、`/cloud_registered`、`/Laser_map`，用于 RViz 点云地图显示和 FAST-LIO2 输入。
...
- `/scan` 和 `/livox/lidar` 是 ROS 侧 transitional emulator，不是 Gazebo GPU lidar。
...
- 动态障碍是 scan 层圆形障碍和 RViz marker，不是 Gazebo 物理移动模型。
...
- 动态障碍已完成 scan-layer smoke；不是 Gazebo 物理动态障碍。
...
```bash
ros2 launch airos_sim sim.launch.py gui:=false rviz:=false
```
