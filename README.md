...
- Gazebo 原生 Livox 风格点云：`gpu_lidar` 输出 `PointCloudPacked`，桥接到 ROS 的 `/livox/lidar`，用于 FAST-LIO2 输入。
...
- `/scan` 和 `/livox/lidar` 现在默认来自 Gazebo 原生传感器；`sensor_source:=emulated` 仍可回退到 ROS 侧模拟器。
