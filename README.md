...
- FAST-LIO2 地形规划入口：`terrain_pct_planner` 可在 `terrain_map_source:=slam_cloud` 下订阅对齐后的 `/Laser_map_world`，从 FAST-LIO2 点云地图抽取 `/terrain_traversability_cloud`，并为已连通的已建图区域发布 `/pct_path`。
...
- FAST-LIO2 `/Laser_map_world` 到 terrain planner 的链路已验证为同层可规划，并已具备跨层目标的前沿探索机制：高层目标暂不可达时，规划器会保留最终目标，先发布当前 FAST-LIO2 已建图连通区域内的 `/pct_path`，并在检测到高处已建图结构时把探索方向偏向该结构。最新运行已能从实时 FAST-LIO 地图生成接近高层目标的高程路径，但机器人最终运动到达仍未验收，不能声称已经完成专题级全链路复现。
