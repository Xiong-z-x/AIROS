...
TERRAIN_MAX_STEP_HEIGHT="${TERRAIN_MAX_STEP_HEIGHT:-0.50}"
...
ROBOT_MOBILITY_PROFILE="${ROBOT_MOBILITY_PROFILE:-legged_champ}"
LEGGED_CHAMP_CONTROLLER="${LEGGED_CHAMP_CONTROLLER:-true}"
LEGGED_CMD_VEL_TOPIC="${LEGGED_CMD_VEL_TOPIC:-/cmd_vel_champ}"
...
run ros2 launch airos_experiments visual_fast_lio_navigation.launch.py \
  ...
  terrain_max_step_height:="$TERRAIN_MAX_STEP_HEIGHT" \
  ...
  legged_champ_controller:="$LEGGED_CHAMP_CONTROLLER" \
  legged_cmd_vel_topic:="$LEGGED_CMD_VEL_TOPIC" \
